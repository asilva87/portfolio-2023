...
export class RomanNumeralsConverterComponent {
  ...
  public onToggleChange() {
    console.log(this.useApostrophus)
  }
  ...
  private getRomanNumber(numberObj: Record<string, number>): string {
    ...
    for (let decimalPlace of decimalPlaces) {
      ...
      if (
        ...
        return `${
          decimalPlace === nextDecimalPlace ? indoRoman[halfNextDecimalPlace] : ''
  ...
  private getNumberDecimalPlaces(number: number): number[] {
  ...
  private repeatNumber(number: string, repetitions: number): string {
    let repeatedNumber = ''
